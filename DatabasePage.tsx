...
import { 
    ...
    FirebaseLogo, PitchutchaLogo, HerokuLogo, ShieldCheckIcon
} from './constants';
...
const DatabaseCodeAnimation: React.FC = () => {
    const { lines, currentLine } = useTypingAnimation(databaseCodeAnimationCommands, true);
             ...
             <div className="p-4">
                {lines.map((line, index) => (
                    <div key={index} className="flex items-center">
                        <span className={`mr-2 ${line.startsWith('✔') ? 'text-green-400' : (line.startsWith('--') ? 'text-gray-500' : 'text-zinc-500')}`}>{line.startsWith('✔') ? '' : '>'}</span>
                        <span className={line.startsWith('✔') ? 'text-green-400' : (line.startsWith('--') ? 'text-gray-500' : 'text-zinc-300')}>{line}</span>
                    </div>
                ))}
                {currentLine && (
...
const RlsSection: React.FC = () => {
    const rlsCode = (
        <>
            <span className="text-gray-500">-- 1. Habilite RLS na tabela 'perfis'</span><br/>
            <span className="text-purple-400">ALTER TABLE</span> perfis <span className="text-yellow-400">ENABLE ROW LEVEL SECURITY;</span><br/><br/>
            <span className="text-gray-500">-- 2. Permita que usuários leiam seus próprios perfis</span><br/>
            <span className="text-purple-400">CREATE POLICY</span> <span className="text-green-400">"Usuários podem ver seu próprio perfil."</span><br/>
            <span className="text-purple-400">ON</span> perfis <span className="text-purple-400">FOR SELECT</span><br/>
            <span className="text-purple-400">USING</span> ( auth.<span className="text-yellow-400">uid</span>() = id );<br/><br/>
            <span className="text-gray-500">-- 3. Permita que usuários atualizem seus próprios perfis</span><br/>
            <span className="text-purple-400">CREATE POLICY</span> <span className="text-green-400">"Usuários podem atualizar seu próprio perfil."</span><br/>
            <span className="text-purple-400">ON</span> perfis <span className="text-purple-400">FOR UPDATE</span><br/>
            <span className="text-purple-400">USING</span> ( auth.<span className="text-yellow-400">uid</span>() = id );
        </>
    );
